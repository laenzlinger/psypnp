...
python_scripts_folder = os.path.join(
    scripting.getScriptsDirectory().toString(), "..", "lib"
)
...
ImageMargins = 1500
FontSize = 32
FontSpacingShrink = 12
IncludeDisabledOfSamePart = False
FontStyle = "font-family: MesloLGS Nerd Font Propo;"
...
def main():
    if not SVGWriteImported:
        psypnp.showMessage(
            "Could not import svgwrite lib, aborting\n%s" % SVGWriteImportError
        )
        return
...
def generate_feed(dwg, aFeed, x_realoffset, y_realoffset, imgdimX, imgdimY):
    ...
    base_y = coord_flip_y(
        map_coord_to_imagespace(aFeed.location.getY(), y_realoffset), imgdimY
    )

    print(
        "Base coords for feed %s are (%s,%s)" % (aFeed.name, str(base_x), str(base_y))
    )
    ...
    boxlen = text_size + FontSize + (5 * FontSpacingShrink)
    arrowpoints = []
    if aFeed.deltaY == 0:
        # horizontal
        if aFeed.deltaX < 0:
            ...
            boxpos = [base_x + 2 * FontSize - boxlen, base_y - 1.5 * FontSize]
            arrowpoints = [
            ...
            dx = distance_per_letter()
            boxpos = [base_x - 2 * FontSize, base_y - 1.5 * FontSize]
            arrowpoints = [
                (base_x - ArrowOffset, base_y),
                (base_x - ArrowOffset + delta_arrow, base_y - delta_arrow),
                (base_x - ArrowOffset, base_y - (2 * delta_arrow)),
        ...
        if aFeed.deltaY < 0:
            ...
            rot = 90
            boxpos = [base_x, base_y - 2 * FontSize]
            arrowpoints = [
            ...
            rot = 90
            boxpos = [base_x - 2, base_y - boxlen + 2 * FontSize]
            arrowpoints = [
                (base_x, base_y + ArrowOffset),
                (base_x + delta_arrow, base_y + ArrowOffset - delta_arrow),
                (base_x + (2 * delta_arrow), base_y + ArrowOffset),
    ...
    for letter_count in range(0, len(feedTxt) + 1):
        xcoords.append(base_x + 0.5 * FontSize + (letter_count * dx))
        ycoords.append(base_y - 0.5 * FontSize + (letter_count * dy))
    ...
    if len(arrowpoints):
        arrlines = dwg.add(
            dwg.g(id="arrow-%i" % aFeed.fid, stroke_width=2, stroke=ArrowColour)
        )
        lastPoint = None
...
def generate_image(feed_info, projname, fname):
    ...
    print("RANGE: %i,%i - %i,%i" % (x_range[0], y_range[0], x_range[1], y_range[1]))
